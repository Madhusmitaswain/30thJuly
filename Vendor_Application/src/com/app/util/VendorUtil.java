...
public class VendorUtil {
	...
	public List<String> getVenIdTypes(){
		return Arrays.asList("PAN CARD","VOTER ID","OTHER");
	}
...
public void generatePie(String path,List<Object[]> data){
		...
		//2. JFreeCharts
		JFreeChart chart=ChartFactory.createPieChart3D("LOCATION WISE VENDORS", dataset, true, true, false);
		//3. Image
		try {
			ChartUtilities.saveChartAsJPEG(new File(path+"/venReportA.jpg"), chart, 500, 500);
		} catch (IOException e) {
			e.printStackTrace();
		}
}
	public void generateBar(String path,List<Object[]> data){
		...
		JFreeChart chart=ChartFactory.createBarChart3D("LOCATION WISE VENDORS", "VENDOR TYPE", "COUNT", dataset);
		
		try{
			ChartUtilities.saveChartAsJPEG(new File(path+"/venReportB.jpg"),chart,500,500);
		}catch(Exception e){
